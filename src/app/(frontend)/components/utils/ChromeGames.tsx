...
const ChromeDinoGame: React.FC<{ options?: GameOptions }> = ({
	options: propOptions,
}) => {
	...
	const draw = useCallback(() => {
		...
		gameOptions.current.groundOffset =
			gameOptions.current.groundOffset! < canvas.width
				...
				: gameOptions.current.groundOffset! - canvas.width;
		ctx.translate(
			gameOptions.current.skyOffset! - gameOptions.current.groundOffset!,
			0
		);
		...
		if (jumpHeight.current <= 1) {
			...
			jumpDelta.current =
				jumpHeight.current ** 2 * 0.001033 - jumpHeight.current * 0.137 + 5;
	...
	useEffect(() => {
		...
		return () => {
			...
			if (canvas)
				canvas.parentNode?.removeEventListener("click", handleSpacePress);
		};
