  ...
  for (let i = 0; i < count; i++) {
    try {
      ...
      if (lengthConstraints.min && lengthConstraints.max) {
        ...
        const targetLength = Math.floor(Math.random() * (lengthConstraints.max - lengthConstraints.min + 1)) + lengthConstraints.min
 ...
 */
export function generatePracticeText(options: WordGenerationOptions & { wordCount?: number } = {}): string {
...
/**
 * Legacy compatibility function for existing keystroke generation
 */
